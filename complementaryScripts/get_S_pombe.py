...
def get_sequence(genes) :
    ...
    with open("../Data/geneseq/S_pombe.cds.fasta", "rU") as handleGene :
        geneSeq = dict()
    ...
    with open("../Data/proteinseq/S_pombe.peptide.fasta", "rU") as handleProtein :
        proteinSeq = dict()
        for record in SeqIO.parse(handleProtein, "fasta") :
            proteinSeq[record.id[:-4]] = str(record.seq)   # record id example: SPAC1002.03c:pep, what we need is SPAC1002.03c
...
def main() :
    ...
    genes = read_xls("S. pombe.xlsx")
    geneAll = get_sequence(genes)

    # write geneAll list into processed data folder
    with open("../Data/processed_data/S_pombe.json", "w") as outfile :
