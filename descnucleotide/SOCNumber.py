...
def SOCNumber(fastas, nlag=30, **kw):
    ...
    for i in fastas:
        name, label = i[0], i[2]
        sequence = getProSeq(name).replace("*", "")
        code = [name, label]
        for n in range(1, nlag + 1):
            try :
                code1 =sum([AADistance[DictAA[sequence[j]]][DictAA[sequence[j + n]]] ** 2 for j in range(len(sequence) - n)]) / (len(sequence) - n)
            except :
                code1 = 0.35
        ...
        for n in range(1, nlag + 1):
            try :
                code2 = sum([AADistance1[DictAA1[sequence[j]]][DictAA1[sequence[j + n]]] ** 2 for j in range(len(sequence) - n)]) / (len(sequence) - n)
            except :
                code2 = 9555
